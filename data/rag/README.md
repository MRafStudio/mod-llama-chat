...
This directory contains sample JSON data files for the Llama Chat RAG (Retrieval-Augmented Generation) system. These files provide comprehensive World of Warcraft knowledge that the bot can use to enhance its responses with relevant game information.
...
The RAG system is controlled by configuration options in `mod_llama.conf.dist`:
- `RAG.Enable`: Enable/disable the RAG system
- `RAG.DataPath`: Path to the data directory (default: "modules/mod-llama/data/rag/")
