...
if(TARGET modules)
    ...
    if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/deps/nlohmann/json.hpp")
        ...
        message(STATUS "[mod-llama-wow] Using bundled nlohmann/json")
    elseif(EXISTS "${CMAKE_SOURCE_DIR}/deps/nlohmann")
        target_include_directories(modules PRIVATE ${CMAKE_SOURCE_DIR}/deps/nlohmann)
        message(STATUS "[mod-llama-wow] Using AzerothCore deps nlohmann/json")
    else()
        find_package(nlohmann_json CONFIG QUIET)
        if(nlohmann_json_FOUND)
            target_link_libraries(modules PRIVATE nlohmann_json::nlohmann_json)
            message(STATUS "[mod-llama-wow] Using system nlohmann/json")
        else()
            message(FATAL_ERROR "[mod-llama-wow] nlohmann/json not found. The module includes a bundled version in deps/nlohmann/json.hpp - please ensure this file exists")
        endif()
    endif()
    ...
    if(TARGET fmt)
        ...
        message(STATUS "[mod-llama-wow] Using AzerothCore fmt library")
    else()
        # Try to find fmt via find_package (vcpkg/system packages)
        find_package(fmt CONFIG QUIET)
        if(fmt_FOUND)
            target_link_libraries(modules PRIVATE fmt::fmt)
            message(STATUS "[mod-llama-wow] Using system fmt library")
        else()
            message(FATAL_ERROR "[mod-llama-wow] fmt library not found. Please install it:\n"
    ...
    if(OpenSSL_FOUND OR OPENSSL_FOUND)
        ...
        message(STATUS "[mod-llama-wow] OpenSSL found - HTTPS support enabled")
    else()
        message(WARNING "[mod-llama-wow] OpenSSL not found - only HTTP support available")
        message(STATUS "[mod-llama-wow] To enable HTTPS: install OpenSSL development libraries")
        if(WIN32)
            message(STATUS "[mod-llama-wow] Windows: Install vcpkg and run 'vcpkg install openssl'")
        else()
            message(STATUS "[mod-llama-wow] Linux: apt install libssl-dev (Ubuntu/Debian) or yum install openssl-devel (RHEL/CentOS)")
        endif()
    endif()
