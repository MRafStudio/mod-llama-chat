...
#include "mod-llama-wow_openai.h"   // [MRafStudio fork] OpenAI-совместимый транспорт
...
namespace
{
    ...
    thread_local LlamaWowHttpClient t_httpClient;

    std::mutex             g_settingsMutex;
    LlamaWowEndpointSettings g_settings;

    nlohmann::json BuildRequest(const LlamaWowEndpointSettings& cfg,
                                const std::string& prompt,
                                const LlamaWowThinkRequest& think,
                                uint32_t reasoningReserve)
    {
        ...
        if (!cfg.seed.empty())
        {
            ...
            {
                if (g_DebugEnabled)
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Invalid seed value: {}", cfg.seed);
            }
        }
        ...
        // disabling it. The old code also sent "hidethinking", which LlamaWow
    ...
    // LlamaWow answers non-streaming requests with a single JSON object, but the
                                ...
                                uint32_t reasoningReserve)
    {
        ...
        // уходит на /v1/chat/completions (llama.cpp, vLLM, LlamaWow>=0.2, DeepSeek,
        // LM Studio...) вместо нативного /api/generate. Внешний мост-переводчик
        // (отдельный процесс) больше не нужен.
        if (LlamaWowOpenAi::IsEnabled(cfg))
            return LlamaWowOpenAi::PerformOnce(cfg, prompt, think.level, reasoningReserve);

        const nlohmann::json request = BuildRequest(cfg, prompt, think, reasoningReserve);

        const auto started = std::chrono::steady_clock::now();
        LlamaWowHttpResult http = t_httpClient.PostEx(cfg.url, request.dump());
...
{
    LlamaWowEndpointSettings next;
    next.url              = g_LlamaWowUrl;
    next.model            = g_LlamaWowModel;
    next.systemPrompt     = g_LlamaWowSystemPrompt;
    next.stop             = g_LlamaWowStop;
    next.seed             = g_LlamaWowSeed;
    next.numPredict       = g_LlamaWowNumPredict;
    next.numCtx           = g_LlamaWowNumCtx;
    next.numThreads       = g_LlamaWowNumThreads;
    next.temperature      = g_LlamaWowTemperature;
    next.topP             = g_LlamaWowTopP;
    next.repeatPenalty    = g_LlamaWowRepeatPenalty;
    next.topK             = g_LlamaWowTopK;
    next.minP             = g_LlamaWowMinP;
    next.presencePenalty  = g_LlamaWowPresencePenalty;
    next.frequencyPenalty = g_LlamaWowFrequencyPenalty;

    // [MRafStudio fork] протокол и токен
    next.apiMode               = g_LlamaWowApiMode;
    next.openAiKey             = g_LlamaWowApiKey;
    next.openAiDisableThinking = g_LlamaWowOpenAiDisableThinking;
...
LlamaWowApiResult QueryLlamaWow(const std::string& prompt, LlamaWowRequestKind kind)
{
    ...
    const LlamaWowEndpointSettings cfg = LlamaWowConfig_Snapshot();
    ...
    LlamaWowThinkRequest think = LlamaWowCapability_ResolveThink(kind);
    ...
    const bool expectReasoning = think.wanted || LlamaWowCapability_ReasonsUnconditionally();
    ...
    auto perform = [&](LlamaWowThinkRequest& req, uint32_t budget)
    {
        LlamaWowApiResult r = PerformOnce(cfg, prompt, req, budget);

        // LlamaWow would not take a string reasoning level. Drop to the boolean
        // form, remember it for this model, and answer rather than lose this
        // request.
        if (!r.ok && !req.level.empty() && r.status >= 400 && r.status < 500)
        {
            LlamaWowCapability_NoteEffortLevelRejected();
            req.level.clear();
    ...
    if (!result.ok && think.wanted &&
        LlamaWowCapability_IsThinkRejection(result.status, result.error))
    {
        LlamaWowCapability_NoteThinkRejected();
        think  = LlamaWowThinkRequest{};
    ...
    {
        if (!think.wanted)
            LlamaWowCapability_NoteUnconditionalReasoning();

        think  = LlamaWowCapability_ResolveThink(kind);
    ...
    if (result.ok && result.text.empty() && !result.thinking.empty())
    {
        LOG_ERROR("module.mod_llama_wow",
                  "[LlamaWow Chat] Model '{}' returned {} characters of reasoning and no answer. "
    ...
    if (result.ok)
        LlamaWowCapability_NoteLatency(result.latencyMs, result.thinkUsed);

    if (!result.ok)
    {
        LOG_ERROR("module.mod_llama_wow",
                  "[LlamaWow Chat] Generation failed (model '{}', {}ms): {}",
    ...
    else if (g_DebugEnabled)
    {
        ...
        LOG_INFO("module.mod_llama_wow",
                 "[LlamaWow Chat] Generation ok in {}ms (think={}), {} chars.",
                 result.latencyMs, thinkText, result.text.size());

        if (g_DebugShowFullPrompt && !result.thinking.empty())
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Model reasoning: {}", result.thinking);
    }
...
{
    LlamaWowApiResult r = QueryLlamaWow(prompt, LlamaWowRequestKind::ChatReply);
