...
void SetBotPlayerSentiment(uint64_t botGuid, uint64_t playerGuid, float sentimentValue)
{
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Set sentiment between bot {} and player {} to {:.2f}", 
...
float AnalyzeMessageSentiment(const std::string& prompt)
{
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Sentiment analysis prompt: {}", prompt);
    }
    ...
    LlamaWowApiResult api = QueryLlamaWow(prompt, LlamaWowRequestKind::Sentiment);
    std::string response = api.ok ? api.text : std::string();

    if (response.empty())
    {
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Empty sentiment analysis response");
        return 0.0f;
    }
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Sentiment analysis: '{}' -> adjustment: {:.2f}", 
                 response, adjustment);
    }
                            ...
                            const std::string& message, const std::string& prompt)
{
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow",
                 "[LlamaWow Chat] Sentiment {:.2f} -> {:.2f} ({:+.2f}) for bot {} toward player {}",
...
void UpdateBotPlayerSentiment(Player* bot, Player* player, const std::string& message)
{
    ...
    LlamaWowDispatch_SubmitSentiment(bot->GetGUID().GetRawValue(),
...
void LoadBotPlayerSentimentsFromDB()
{
    ...
    QueryResult result = CharacterDatabase.Query("SELECT bot_guid, player_guid, sentiment_value FROM mod_llama_wow_bot_player_sentiments");
    
    if (!result)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] No existing sentiment data found in database");
        return;
    }
    ...
    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Loaded {} sentiment records from database", count);
}
...
void SaveBotPlayerSentimentsToDB()
{
    ...
    for (const auto& [key, sentimentValue] : changed)
    {
        trans->Append(SafeFormat(
            "INSERT INTO mod_llama_wow_bot_player_sentiments "
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Saved {} changed sentiment record(s) to database",
...
void InitializeSentimentTracking()
{
    if (!g_EnableSentimentTracking)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Sentiment tracking is disabled");
        return;
    }
    
    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Initializing sentiment tracking system...");
    ...
    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Sentiment tracking system initialized");
}
