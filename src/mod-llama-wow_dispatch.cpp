...
namespace
{
    ...
    {
        LlamaWowApiResult api = QueryLlamaWow(task.request.prompt, task.request.kind);
        ...
        if (!text.empty())
        {
            ...
            if (filtered.empty() && !text.empty() && g_DebugEnabled)
            {
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} reply rejected by roleplay filter: '{}'",
        ...
        if (text.empty())
        {
            ...
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} produced nothing usable after cleanup.",
    ...
    void WorkerLoop()
    {
        for (;;)
        {
            ...
            {
                RecordError(e.what());
                LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] Worker exception: {}", e.what());
            }
            catch (...)
            {
                RecordError("unknown exception");
                LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] Unknown worker exception.");
            }
    ...
    bool RouteMessage(Player* bot, PlayerbotAI* botAI, const Completion& c,
                      const LlamaWowWorldSnapshot& world,
                      Channel*& outChannel)
    {
        outChannel = nullptr;
        ...
        switch (c.request.source)
        {
            ...
            case SRC_PARTY_LOCAL:
                if (g_DisableForParty || !bot->GetGroup())
                    return false;
                if (!LlamaWowGroupHasRealPlayer(bot))
                    return false;
    ...
    void Deliver(const Completion& c, const LlamaWowWorldSnapshot& world)
    {
        ...
        if (!directAddress && Governor_IsRepetitive(botGuid, c.request.scopeKey, c.text))
        {
            ...
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} reply suppressed as repetitive: '{}'",
                         bot->GetName(), c.text);
            return;
        }
        ...
        if (!Governor_TryConsumeSend(botGuid, c.request.scopeKey, directAddress))
        {
            ...
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} reply suppressed by cooldown/rate limit.",
                         bot->GetName());
            return;
        }
        ...
        if (!RouteMessage(bot, botAI, c, world, channel))
        {
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} had nowhere to send its reply ({}).",
        ...
        if (c.request.targetGuid)
        {
            ...
            if (LlamaWowIsRealPlayer(addressee))
        ...
        {
            LlamaWowDispatch_SubmitSentiment(c.request.botGuid, c.request.targetGuid,
                                           c.request.originMessage);
        }

        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] {} ({}, depth {}): {}",
                     bot->GetName(), ChatChannelSourceLocalStr[c.request.source],
...
void LlamaWowDispatch_Start()
{
    ...
    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Dispatcher started with {} worker threads.", count);
}

void LlamaWowDispatch_Stop()
{
    {
        std::lock_guard<std::mutex> lock(g_queueMutex);
        if (!g_running)
    ...
    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Dispatcher stopped.");
}

bool LlamaWowDispatch_Submit(LlamaWowChatRequest request)
{
...

    {
        ...
        if (g_MaxQueueDepth > 0 && g_queue.size() >= g_MaxQueueDepth)
        {
            ...
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Queue full ({}); dropping request for {}.",
...
void LlamaWowDispatch_SubmitSentiment(uint64_t botGuid, uint64_t playerGuid,
...
void LlamaWowDispatch_Update(uint32_t /*diff*/)
{
    ...
    for (const Completion& c : due)
    {
        ...
        {
            LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] Delivery exception: {}", e.what());
        }
    }
}

void LlamaWowChat_DispatchEmoteReaction(Player* bot, Player* player, uint32_t textEmote)
{
    if (!bot || !player)
        return;
    ...
    request.kind       = LlamaWowRequestKind::EventChatter;
    ...
    LlamaWowDispatch_Submit(std::move(request));
}
...
void LlamaWowDispatch_SubmitCondensation(uint64_t botGuid, const std::string& prompt)
{
...
void LlamaWowDispatch_SubmitRelationship(uint64_t botGuid, uint64_t otherGuid,
...
LlamaWowDispatchStats LlamaWowDispatch_GetStats()
{
    LlamaWowDispatchStats stats{};
...
Channel* LlamaWowResolveZoneChannel(Player* bot, uint32_t chatChannelId)
{
    if (!bot)
        return nullptr;
