...
namespace
{
    LlamaWowBotEventChatter eventChatter;
...
void LlamaWowBotEventChatter::DispatchGameEvent(Player* source, std::string type, std::string detail)
{
    ...
    const bool sourceIsBot = LlamaWowIsBotPlayer(source);
    ...
    if (g_DebugEnabled)
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Event from {}: type={} detail={}",
    ...
    for (Player* bot : candidateBots)
    {
        ...
        const bool partyAudience =
            bot->GetGroup() && !g_DisableForParty && LlamaWowGroupHasRealPlayer(bot);
        ...
        if (!LlamaWowDispatch_Submit(std::move(request)))
            continue;
    ...
    if (g_DebugEnabled)
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Event dispatch complete, {} bots queued.", responses);
}

std::string LlamaWowBotEventChatter::BuildPrompt(Player* bot, std::string promptTemplate,
                                               ...
                                               std::string actorName)
{
    ...
    Player* evMaster                    = ai->GetMaster();
    uint8   evLocale                    = (evMaster && evMaster->GetSession()) ? evMaster->GetSession()->GetSessionDbcLocale() : uint8(LOCALE_enUS);
    const std::string personalityPrompt = GetPersonalityPromptAddition(personality, evLocale);
    ...
    std::string prompt = SafeFormat(
        ...
        fmt::arg("bot_map", LlamaWowContinentName(bot)),
...
void ChatOnQuest::OnPlayerCompleteQuest(Player* player, Quest const* quest)
{
        ...
        player->GetMap() && player->GetMap()->IsDungeon())
    {
        eventChatter.DispatchGameEvent(
            ...
            SafeFormat("{} in {}", quest->GetTitle(), LlamaWowContinentName(player)));
    }
}
