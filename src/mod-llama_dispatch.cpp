...
namespace
{
    ...
    {
        LlamaApiResult api = QueryLlama(task.request.prompt, task.request.kind);
    ...
    bool RouteMessage(Player* bot, PlayerbotAI* botAI, const Completion& c,
                      const LlamaWorldSnapshot& world,
                      Channel*& outChannel)
    {
        outChannel = nullptr;
        ...
        switch (c.request.source)
        {
            ...
            case SRC_PARTY_LOCAL:
                if (g_DisableForParty || !bot->GetGroup())
                    return false;
                if (!LlamaGroupHasRealPlayer(bot))
                    return false;
    ...
    void Deliver(const Completion& c, const LlamaWorldSnapshot& world)
    {
        ...
        if (c.request.targetGuid)
        {
            ...
            if (LlamaIsRealPlayer(addressee))
        ...
        {
            LlamaDispatch_SubmitSentiment(c.request.botGuid, c.request.targetGuid,
...
void LlamaDispatch_SubmitSentiment(uint64_t botGuid, uint64_t playerGuid,
...
void LlamaDispatch_Update(uint32_t /*diff*/)
{
...
void LlamaChat_DispatchEmoteReaction(Player* bot, Player* player, uint32_t textEmote)
{
    if (!bot || !player)
        return;
    ...
    request.kind       = LlamaRequestKind::EventChatter;
    ...
    LlamaDispatch_Submit(std::move(request));
}
...
void LlamaDispatch_SubmitCondensation(uint64_t botGuid, const std::string& prompt)
{
...
void LlamaDispatch_SubmitRelationship(uint64_t botGuid, uint64_t otherGuid,
...
LlamaDispatchStats LlamaDispatch_GetStats()
{
    LlamaDispatchStats stats{};
...
Channel* LlamaResolveZoneChannel(Player* bot, uint32_t chatChannelId)
{
    if (!bot)
        return nullptr;
