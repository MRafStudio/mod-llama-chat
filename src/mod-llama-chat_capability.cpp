...
namespace
{
    ...
    LlamaThinkSupport g_support = LlamaThinkSupport::Unknown;
    ...
    void SetSupport(LlamaThinkSupport support, const std::string& detail)
    {
    ...
    // Step 1: ask Llama what the model can do. Llama 0.6+ returns a
    // "capabilities" array on /api/show.
    // Returns true when the answer was conclusive.
    bool ProbeViaShow(LlamaHttpClient& http, const std::string& base,
                      const std::string& model, bool& outSupported,
                      std::string& outDetail)
    {
        nlohmann::json body = { { "model", model } };

        LlamaHttpResult r = http.PostEx(base + "/api/show", body.dump(),
        ...
        try
        {
            ...
            if (!parsed.contains("capabilities") || !parsed["capabilities"].is_array())
                return false;   // older Llama: fall through to the live probe
    ...
    // Step 2 (older Llama, or an inconclusive /api/show): actually ask for a
    // one-token thinking generation and see whether it is refused.
    bool ProbeViaGenerate(LlamaHttpClient& http, const std::string& base,
                          const std::string& model, bool& outSupported,
                          std::string& outDetail)
    {
        nlohmann::json body = {
        ...
        LlamaHttpResult r = http.PostEx(base + "/api/generate", body.dump(),
                                         static_cast<int>(g_CapabilityProbeTimeoutSeconds));
        ...
        if (LlamaCapability_IsThinkRejection(r.status, r.body))
        {
            outSupported = false;
...
void LlamaCapability_Init(bool force)
{
    ...
    if (g_LlamaApiMode == "openai" || g_LlamaApiMode == "openai-compatible" || g_LlamaApiMode == "v1")
    {
        SetSupport(LlamaThinkSupport::Unsupported,
                   "openai-compatible mode: /api/show probe skipped");
        return;
    }

    const std::string base  = LlamaDeriveBaseUrl(g_LlamaUrl);
    const std::string model = g_LlamaModel;

    {
        ...
        if (!force && base == g_probedUrl && model == g_probedModel &&
            g_support != LlamaThinkSupport::Unknown)
        {
    ...
    // Off the world thread: a missing Llama must not stall startup.
    std::thread(RunProbe, base, model).detach();
}

LlamaThinkSupport LlamaCapability_GetSupport()
{
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_support;
}

bool LlamaCapability_SupportsThinking()
{
    return LlamaCapability_GetSupport() == LlamaThinkSupport::Supported;
}

bool LlamaCapability_IsThinkRejection(int status, const std::string& body)
{
...
void LlamaCapability_NoteThinkRejected()
{
    SetSupport(LlamaThinkSupport::Unsupported,
...
void LlamaCapability_NoteUnconditionalReasoning()
{
...
bool LlamaCapability_ReasonsUnconditionally()
{
    return g_reasonsUnconditionally.load();
}

void LlamaCapability_NoteEffortLevelRejected()
{
...
bool LlamaCapability_EffortLevelsRejected()
{
    return g_effortLevelsRejected.load();
}

LlamaThinkRequest LlamaCapability_ResolveThink(LlamaRequestKind kind)
{
    LlamaThinkRequest req;
    req.wanted  = LlamaCapability_ShouldThink(kind);
...
void LlamaCapability_NoteLatency(uint64_t milliseconds, bool thinkUsed)
{
...
bool LlamaCapability_ThinkDisabledByLatency()
{
    return g_latencyDisabled.load();
}

bool LlamaCapability_ShouldThink(LlamaRequestKind kind)
{
    const LlamaThinkPolicy policy = static_cast<LlamaThinkPolicy>(g_ThinkModePolicy);

    if (policy == LlamaThinkPolicy::Off)
        return false;
    ...
    if (!LlamaCapability_SupportsThinking())
        return false;

    if (g_latencyDisabled.load())
        return false;

    if (policy == LlamaThinkPolicy::On)
        return true;

    // Auto: spend reasoning only where it changes the answer.
    switch (kind)
    {
        case LlamaRequestKind::Sentiment:
            return true;    // a judgement, and never shown to players
        case LlamaRequestKind::RoleplayReply:
            return g_RoleplayStrictness >= 2;
        case LlamaRequestKind::ChatReply:
        case LlamaRequestKind::RandomChatter:
        case LlamaRequestKind::EventChatter:
        default:
            return false;   // short, latency-sensitive, nothing to reason about
    }
}

std::string LlamaCapability_StatusText()
{
...

    {
        ...
        switch (g_support)
        {
            case LlamaThinkSupport::Supported:   supportText = "supported";   break;
            case LlamaThinkSupport::Unsupported: supportText = "unsupported"; break;
            case LlamaThinkSupport::ProbeFailed: supportText = "probe failed"; break;
    ...
    switch (static_cast<LlamaThinkPolicy>(g_ThinkModePolicy))
    {
        case LlamaThinkPolicy::On:  policyText = "on";  break;
        case LlamaThinkPolicy::Off: policyText = "off"; break;
...
LlamaThinkPolicy LlamaCapability_ParsePolicy(const std::string& text, bool legacyBool)
{
    ...
    if (v == "auto" || v.empty())
    {
        ...
        return legacyBool ? LlamaThinkPolicy::On : LlamaThinkPolicy::Auto;
    }
    if (v == "on" || v == "1" || v == "true" || v == "yes" || v == "always")
        return LlamaThinkPolicy::On;
