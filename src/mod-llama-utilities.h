#ifndef MOD_LLAMA_UTILS_H
#define MOD_LLAMA_UTILS_H
...
// Safe formatting utility for the Llama Chat module.
...
inline std::string SafeFormat(const std::string& templ, Args&&... args) {
    try {
        ...
        LOG_ERROR("server.loading", "[Llama Chat] Format error: {} | Template: {}", e.what(), templ);
        return "[Format Error]";
    }
}
