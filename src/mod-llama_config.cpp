...
// --------------------------------------------
// Llama LLM API Configuration
// --------------------------------------------
std::string g_LlamaUrl        = "http://localhost:11434/api/generate";
std::string g_LlamaModel      = "llama3.2:1b";
uint32_t    g_LlamaNumPredict = 40;
float       g_LlamaTemperature = 0.8f;
float       g_LlamaTopP = 0.95f;
float       g_LlamaRepeatPenalty = 1.1f;
uint32_t    g_LlamaNumCtx = 0;
uint32_t    g_LlamaNumThreads = 0;
std::string g_LlamaStop = "";
std::string g_LlamaSystemPrompt = "";
...
float       g_LlamaMinP             = -1.0f;
float       g_LlamaPresencePenalty  = -1000.0f;
float       g_LlamaFrequencyPenalty = -1000.0f;
...
uint8_t  g_ThinkModePolicy               = 0;      // LlamaThinkPolicy::Auto
...
LlamaRAGSystem* g_RAGSystem = nullptr;
...
void LoadLlamaChatConfig()
{
    ...
    // the g_Llama* globals directly -- reassigning those here would free the
    ...
    LOG_INFO("server.loading",
             ...
             g_MaxBotsToPick,
             g_LlamaUrl, g_LlamaModel, g_MaxConcurrentQueries,
...
LlamaChatConfigWorldScript::LlamaChatConfigWorldScript() : WorldScript("LlamaChatConfigWorldScript") { }

void LlamaChatConfigWorldScript::OnStartup()
{
    LoadLlamaChatConfig();
    ...
    // Ask Llama what this model can actually do. Runs on its own thread so a
    // missing or slow Llama cannot stall worldserver startup; until it
...
void LlamaChatConfigWorldScript::OnShutdown()
{
