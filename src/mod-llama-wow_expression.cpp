    ...
    if (!found && !unresolved.empty())
    {
        ...
        if (g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow",
                     "[LlamaWow Chat] Emote tag '{}' is not a WoW emote; {}.",
                     unresolved,
                                    ...
                                    uint32 /*emoteNum*/, ObjectGuid guid)
{
    ...
    if (reactEmote)
    {
        ...
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} reacting to emote {} from {} with emote {}",
    ...
    if (speak)
    {
        ...
        LlamaWowChat_DispatchEmoteReaction(bot, player, textEmote);
    }
}
