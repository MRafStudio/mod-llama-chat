...
namespace
{
    ...
    ParsedUrl ParseUrl(const std::string& url)
    {
        ...
        {
            // No explicit port. Use the scheme default -- NOT Llama's 11434,
    ...
    httplib::Headers BuildHeaders(const std::string& host)
    {
        httplib::Headers headers = {
            ...
            { "User-Agent",   "AzerothCore-LlamaChat/2.0" },
...
std::string LlamaDeriveBaseUrl(const std::string& url)
{
...
LlamaHttpClient::LlamaHttpClient()
    : m_timeout(120), m_available(true)
{
}

LlamaHttpClient::~LlamaHttpClient() = default;

void LlamaHttpClient::SetTimeout(int seconds)
{
    m_timeout = seconds;
}

bool LlamaHttpClient::IsAvailable() const
{
    return m_available;
}

LlamaHttpResult LlamaHttpClient::PostEx(const std::string& url, const std::string& jsonData,
                                          int timeoutOverride)
{
    LlamaHttpResult result;
...
LlamaHttpResult LlamaHttpClient::GetEx(const std::string& url, int timeoutOverride)
{
    LlamaHttpResult result;
...
std::string LlamaHttpClient::Post(const std::string& url, const std::string& jsonData)
{
    LlamaHttpResult r = PostEx(url, jsonData);
...
LlamaHttpResult LlamaHttpClient::PostExWithToken(const std::string& url, const std::string& jsonData,
