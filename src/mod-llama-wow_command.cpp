...
LlamaWowChatConfigCommand::LlamaWowChatConfigCommand()
    : CommandScript("LlamaWowChatConfigCommand")
{
}

ChatCommandTable LlamaWowChatConfigCommand::GetCommands() const
{
    static ChatCommandTable llamaWowSentimentCommandTable =
    {
        { "view",  HandleLlamaWowSentimentViewCommand,  SEC_ADMINISTRATOR, Console::Yes },
        { "set",   HandleLlamaWowSentimentSetCommand,   SEC_ADMINISTRATOR, Console::Yes },
        { "reset", HandleLlamaWowSentimentResetCommand, SEC_ADMINISTRATOR, Console::Yes }
    };

    static ChatCommandTable llamaWowPersonalityCommandTable =
    {
        { "get",  HandleLlamaWowPersonalityGetCommand,  SEC_ADMINISTRATOR, Console::Yes },
        { "set",  HandleLlamaWowPersonalitySetCommand,  SEC_ADMINISTRATOR, Console::Yes },
        { "list", HandleLlamaWowPersonalityListCommand, SEC_ADMINISTRATOR, Console::Yes }
    };

    static ChatCommandTable llamaWowReloadCommandTable =
    {
        { "reload",      HandleLlamaWowReloadCommand,  SEC_ADMINISTRATOR, Console::Yes },
        { "status",      HandleLlamaWowStatusCommand,  SEC_ADMINISTRATOR, Console::Yes },
        { "test",        HandleLlamaWowTestCommand,    SEC_ADMINISTRATOR, Console::Yes },
        { "sentiment",   llamaWowSentimentCommandTable },
        { "personality", llamaWowPersonalityCommandTable }
    };

    static ChatCommandTable commandTable =
    {
        { "ollama", llamaWowReloadCommandTable }
    };

    return commandTable;
}

bool LlamaWowChatConfigCommand::HandleLlamaWowReloadCommand(ChatHandler* handler)
{
    sConfigMgr->Reload();
    ...
    handler->SendSysMessage("LlamaWowChat: Configuration reloaded from conf!");
    return true;
}

bool LlamaWowChatConfigCommand::HandleLlamaWowSentimentViewCommand(ChatHandler* handler, Optional<std::string> botName, Optional<std::string> playerName)
{
    if (!g_EnableSentimentTracking)
    {
        handler->SendSysMessage("LlamaWowChat: Sentiment tracking is disabled.");
        return true;
    }
    ...
    if (!botName && !playerName)
    {
        ...
        {
            handler->SendSysMessage("LlamaWowChat: No sentiment data found.");
            return true;
        }

        handler->SendSysMessage("LlamaWowChat: All sentiment data:");
    ...
    if (botName)
    {
        ...
        if (!targetBot)
        {
            handler->SendSysMessage(fmt::format("LlamaWowChat: Bot '{}' not found.", *botName));
            return true;
        }
        if (!PlayerbotsMgr::instance().GetPlayerbotAI(targetBot))
        {
            handler->SendSysMessage(fmt::format("LlamaWowChat: Player '{}' is not a bot.", *botName));
            return true;
        }
    }
    ...
    if (playerName)
    {
        ...
        if (!targetPlayer)
        {
            handler->SendSysMessage(fmt::format("LlamaWowChat: Player '{}' not found.", *playerName));
            return true;
        }
    }
    ...
    if (targetBot && targetPlayer)
    {
        ...
        handler->SendSysMessage(fmt::format("LlamaWowChat: Bot '{}' -> Player '{}': {:.3f}", 
    ...
    else if (targetBot)
    {
        ...
        {
            handler->SendSysMessage(fmt::format("LlamaWowChat: No sentiment data found for bot '{}'.", targetBot->GetName()));
            return true;
        }

        handler->SendSysMessage(fmt::format("LlamaWowChat: Sentiment data for bot '{}':", targetBot->GetName()));
    ...
    else if (targetPlayer)
    {
        ...
        bool found = false;
        handler->SendSysMessage(fmt::format("LlamaWowChat: Sentiment data involving player '{}':", targetPlayer->GetName()));
        ...
        if (!found)
        {
            handler->SendSysMessage(fmt::format("LlamaWowChat: No sentiment data found involving player '{}'.", targetPlayer->GetName()));
        }
    }
...
bool LlamaWowChatConfigCommand::HandleLlamaWowSentimentSetCommand(ChatHandler* handler, std::string botName, std::string playerName, float sentimentValue)
{
    if (!g_EnableSentimentTracking)
    {
        handler->SendSysMessage("LlamaWowChat: Sentiment tracking is disabled.");
    ...
    {
        handler->SendSysMessage("LlamaWowChat: Sentiment value must be between 0.0 and 1.0.");
        return true;
    }

    SetBotPlayerSentiment(bot->GetGUID().GetRawValue(), player->GetGUID().GetRawValue(), sentimentValue);
    handler->SendSysMessage(fmt::format("LlamaWowChat: Set sentiment between bot '{}' and player '{}' to {:.3f}.", 
                            botName, playerName, sentimentValue));
    return true;
}

bool LlamaWowChatConfigCommand::HandleLlamaWowSentimentResetCommand(ChatHandler* handler, Optional<std::string> botName, Optional<std::string> playerName)
{
    if (!g_EnableSentimentTracking)
    {
        handler->SendSysMessage("LlamaWowChat: Sentiment tracking is disabled.");
    ...
    if (!botName && !playerName)
    {
        ...
        CharacterDatabase.Execute("DELETE FROM mod_llama_wow_bot_player_sentiments");
        handler->SendSysMessage(fmt::format("LlamaWowChat: Reset all sentiment data ({} records).", count));
        return true;
    }
    ...
    if (targetBot && targetPlayer)
    {
        ...
        handler->SendSysMessage(fmt::format("LlamaWowChat: Reset sentiment between bot '{}' and player '{}' to default ({:.3f}).", 
    ...
    else if (targetBot)
    {
        ...
        if (botIt != g_BotPlayerSentiments.end())
        {
            ...
            CharacterDatabase.Execute(SafeFormat(
                "DELETE FROM mod_llama_wow_bot_player_sentiments WHERE bot_guid = {}", botGuid));
            handler->SendSysMessage(fmt::format("LlamaWowChat: Reset all sentiment data for bot '{}' ({} records).", 
    ...
    else if (targetPlayer)
    {
        ...
        CharacterDatabase.Execute(SafeFormat(
            "DELETE FROM mod_llama_wow_bot_player_sentiments WHERE player_guid = {}", playerGuid));

        handler->SendSysMessage(fmt::format("LlamaWowChat: Reset all sentiment data involving player '{}' ({} records).", 
...
bool LlamaWowChatConfigCommand::HandleLlamaWowPersonalityGetCommand(ChatHandler* handler, std::string botName)
{
    Player* bot = ObjectAccessor::FindPlayerByName(botName);
    if (!bot)
    {
    ...
    handler->SendSysMessage(fmt::format("LlamaWowChat: Bot '{}' has personality '{}'", botName, personality));
    handler->SendSysMessage(fmt::format("  Prompt: {}", prompt));
    
    return true;
}

bool LlamaWowChatConfigCommand::HandleLlamaWowPersonalitySetCommand(ChatHandler* handler, std::string botName, std::string personality)
{
    Player* bot = ObjectAccessor::FindPlayerByName(botName);
    if (!bot)
    {
    ...
    {
        handler->SendSysMessage(fmt::format("LlamaWowChat: Personality '{}' does not exist. Use '.llamawow personality list' to see available personalities.", personality));
        return true;
    }
    ...
    {
        std::string prompt = GetPersonalityPromptAddition(personality, handler->GetSessionDbcLocale());
        handler->SendSysMessage(fmt::format("LlamaWowChat: Set bot '{}' personality to '{}'", botName, personality));
        handler->SendSysMessage(fmt::format("  Prompt: {}", prompt));
    }
    else
    {
        handler->SendSysMessage(fmt::format("LlamaWowChat: Failed to set personality for bot '{}'.", botName));
    }
    
    return true;
}

bool LlamaWowChatConfigCommand::HandleLlamaWowPersonalityListCommand(ChatHandler* handler)
{
    std::vector<std::string> personalities = GetAllPersonalityKeys();
    
    if (personalities.empty())
    {
        handler->SendSysMessage("LlamaWowChat: No personalities loaded.");
        return true;
    }
    
    handler->SendSysMessage(fmt::format("LlamaWowChat: Available personalities ({} total, {} random-assignable):", 
...
bool LlamaWowChatConfigCommand::HandleLlamaWowStatusCommand(ChatHandler* handler)
{
    const LlamaWowDispatchStats dispatch = LlamaWowDispatch_GetStats();
    const GovernorStats       gov      = Governor_GetStats();

    handler->PSendSysMessage("|cff00ff00[LlamaWow Chat] Status|r");
    handler->PSendSysMessage("Module: {}   Endpoint: {}   Model: {}",
                             g_Enable ? "enabled" : "DISABLED",
                             g_LlamaWowUrl, g_LlamaWowModel);
    handler->PSendSysMessage("Think: {}", LlamaWowCapability_StatusText());
...
bool LlamaWowChatConfigCommand::HandleLlamaWowTestCommand(ChatHandler* handler, Acore::ChatCommands::Tail prompt)
{
    std::string text(prompt);
    if (text.empty())
    {
        handler->SendSysMessage("Usage: .llamawow test <prompt>");
        handler->SetSentErrorMessage(true);
        return false;
    }

    handler->PSendSysMessage("[LlamaWow Chat] Sending test prompt, please wait...");
    ...
    std::thread([text]()
    {
        LlamaWowApiResult api = QueryLlamaWow(text, LlamaWowRequestKind::ChatReply);

        if (!api.ok)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] TEST FAILED after {}ms: {}",
        ...
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] TEST ok in {}ms (think={}).",
                 api.latencyMs, api.thinkUsed ? "yes" : "no");
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] TEST raw     : {}", api.text);
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] TEST cleaned : {}", cleaned);
        if (emote)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] TEST emote   : {}", emote);
        if (!api.thinking.empty())
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] TEST thinking: {}", api.thinking);
    }).detach();

    handler->PSendSysMessage("[LlamaWow Chat] Result will appear in the server log (module.mod_llama_wow).");
    return true;
}
