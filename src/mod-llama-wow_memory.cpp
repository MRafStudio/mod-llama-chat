...
namespace
{
    ...
    std::unordered_map<uint64_t, LlamaWowMemoryState> g_state;
...
void Memory_Load()
{
    ...
    if (QueryResult result = CharacterDatabase.Query(
            ...
            "FROM mod_llama_wow_memories ORDER BY importance DESC"))
    {
        uint32_t loaded = 0;
        do
        {
        ...
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Loaded {} bot memories.", loaded);
    }

    if (QueryResult result = CharacterDatabase.Query(
            "SELECT bot_guid, other_guid, other_name, description, mentions, "
            "UNIX_TIMESTAMP(updated_at) FROM mod_llama_wow_relationships"))
    {
        uint32_t loaded = 0;
        do
        {
        ...
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Loaded {} bot relationships.", loaded);
    }
}
...
void Memory_SaveAll()
{
    ...
    for (auto& [botGuid, state] : g_state)
    {
        ...
        trans->Append(SafeFormat(
            "DELETE FROM mod_llama_wow_memories WHERE bot_guid = {}", botGuid));
        ...
        if (!values.empty())
        {
            trans->Append("INSERT INTO mod_llama_wow_memories "
        ...
        for (const auto& [otherGuid, r] : state.relationships)
        {
            ...
            trans->Append(SafeFormat(
                "INSERT INTO mod_llama_wow_relationships "
                         ...
                         const std::string& botReply)
{
    ...
    if (g_RelationshipEnable && otherGuid != 0 && !otherName.empty())
    {
        ...
        {
            std::lock_guard<std::mutex> lock(g_mutex);
            LlamaWowMemoryState& state = g_state[botGuid];
        ...
        if (trigger)
        {
            ...
            if (!prompt.empty())
                LlamaWowDispatch_SubmitRelationship(botGuid, otherGuid, otherName, prompt);
            else
            {
                std::lock_guard<std::mutex> lock(g_mutex);
    ...
    LlamaWowDispatch_SubmitCondensation(botGuid, prompt);
}
...
void Memory_RunCondensation(uint64_t botGuid, const std::string& prompt)
{
    ...
    if (!api.ok)
    {
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow",
                     "[LlamaWow Chat] Memory condensation failed for bot {}: {}",
                     botGuid, api.error);
    ...
    if (g_DebugEnabled)
        LOG_INFO("module.mod_llama_wow",
                 "[LlamaWow Chat] Condensed history for bot {} into {} memories.",
                 botGuid, fresh.size());
}
                                  ...
                                  const std::string& prompt)
{
    ...
    if (g_DebugEnabled)
        LOG_INFO("module.mod_llama_wow",
                 "[LlamaWow Chat] Bot {} relationship with {} updated: {}",
