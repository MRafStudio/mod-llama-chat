...
namespace
{
    ...
    bool ChooseDestination(Player* bot, const LlamaWowWorldSnapshot& world, bool guildTopic,
                           ...
                           uint32_t& outChannelId)
    {
        ...
        if (bot->GetGroup() && !g_DisableForParty && LlamaWowGroupHasRealPlayer(bot))
        {
        ...
        if (!g_DisableForCustomChannels && world.RealPlayerInZoneAndFaction(bot))
        {
            ...
            auto addChannel = [&](uint32_t chanId, bool enabled)
            {
                ...
                Channel* ch = LlamaWowResolveZoneChannel(bot, chanId);
                if (!ch)
                    return;
    ...
    std::string BuildRandomChatterPrompt(Player* bot, const std::string& environmentInfo)
    {
        ...
        Player* rcMaster                    = botAI->GetMaster();
        uint8   rcLocale                    = (rcMaster && rcMaster->GetSession()) ? rcMaster->GetSession()->GetSessionDbcLocale() : uint8(LOCALE_enUS);
        const std::string personalityPrompt = GetPersonalityPromptAddition(personality, rcLocale);
...
void LlamaWowRandomChatter_ForgetBot(ObjectGuid botGuid)
{
    std::lock_guard<std::mutex> lock(g_scheduleMutex);
    g_nextRandomChatTime.erase(botGuid.GetRawValue());
}

LlamaWowBotRandomChatter::LlamaWowBotRandomChatter() : WorldScript("LlamaWowBotRandomChatter") { }

void LlamaWowBotRandomChatter::OnUpdate(uint32 diff)
{
    if (!g_Enable)
        return;
...
void LlamaWowBotRandomChatter::HandleRandomChatter()
{
    ...
    for (auto const& itr : allPlayers)
    {
        ...
        if (!ChooseDestination(bot, world, topic.isGuildTopic, source, channelName, channelId))
        {
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} has nowhere to speak; skipping ambient line.",
                         bot->GetName());
            reschedule();
            continue;
        }
        ...
        request.kind        = LlamaWowRequestKind::RandomChatter;
