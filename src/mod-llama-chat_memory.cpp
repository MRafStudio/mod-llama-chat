...
namespace
{
    ...
    std::unordered_map<uint64_t, LlamaMemoryState> g_state;
                         ...
                         const std::string& botReply)
{
    ...
    if (g_RelationshipEnable && otherGuid != 0 && !otherName.empty())
    {
        ...
        {
            std::lock_guard<std::mutex> lock(g_mutex);
            LlamaMemoryState& state = g_state[botGuid];
        ...
        if (trigger)
        {
            ...
            if (!prompt.empty())
                LlamaDispatch_SubmitRelationship(botGuid, otherGuid, otherName, prompt);
            else
            {
                std::lock_guard<std::mutex> lock(g_mutex);
    ...
    LlamaDispatch_SubmitCondensation(botGuid, prompt);
}
...
{
    LlamaApiResult api = QueryLlama(prompt, LlamaRequestKind::Sentiment);
