#include "mod-llama-wow_openai.h"
...
namespace
{
    ...
    thread_local LlamaWowHttpClient t_openAiClient;
...
namespace LlamaWowOpenAi
{
                                ...
                                uint32_t reasoningReserve)
    {
        ...
        LlamaWowHttpResult http = t_openAiClient.PostExWithToken(cfg.url, request.dump(), cfg.openAiKey);
        ...
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] OpenAI-compatible reply ok in {}ms (finish_reason={})",
