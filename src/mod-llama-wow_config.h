#ifndef MOD_LLAMA_WOW_CONFIG_H
#define MOD_LLAMA_WOW_CONFIG_H
...
// --------------------------------------------
extern std::string g_LlamaWowUrl;
extern std::string g_LlamaWowModel;
extern uint32_t    g_LlamaWowNumPredict;
extern float       g_LlamaWowTemperature;
extern float       g_LlamaWowTopP;
extern float       g_LlamaWowRepeatPenalty;
extern uint32_t    g_LlamaWowNumCtx;
extern uint32_t    g_LlamaWowNumThreads;
extern std::string g_LlamaWowStop;
extern std::string g_LlamaWowSystemPrompt;
extern std::string g_LlamaWowSeed;

// --- [MRafStudio fork] протокол и токен --------------------------------
extern std::string g_LlamaWowApiMode;               // "ollama" | "openai"
extern std::string g_LlamaWowApiKey;                // Bearer-токен (может быть пустым)
extern bool        g_LlamaWowOpenAiDisableThinking;
...
extern int32_t     g_LlamaWowTopK;              // -1 = unset
extern float       g_LlamaWowMinP;              // -1 = unset
extern float       g_LlamaWowPresencePenalty;   // <= -999 = unset
extern float       g_LlamaWowFrequencyPenalty;  // <= -999 = unset
...
// llama-wow: локализованные промпты личностей: key -> (id локали -> текст)
extern std::unordered_map<std::string, std::unordered_map<uint8, std::string>> g_PersonalityPromptsLocale;
...
// --------------------------------------------
// Think Mode (see mod-llama-wow_capability.h)
// --------------------------------------------
extern bool     g_ThinkModeEnableForModule;   // deprecated; maps onto ThinkMode
extern uint8_t  g_ThinkModePolicy;            // LlamaWowThinkPolicy
...
extern LlamaWowRAGSystem* g_RAGSystem;                     // Global RAG system instance

// --------------------------------------------
// Event Chatter: Event Type Strings
// These control the event type string sent to eventChatter for world event prompts.
// Values are loaded from conf (see mod_llama_wow.conf.dist)
// --------------------------------------------
...
// --------------------------------------------
class LlamaWowChatConfigWorldScript : public WorldScript
{
public:
    LlamaWowChatConfigWorldScript();
    void OnStartup() override;
    void OnShutdown() override;
};

#endif // MOD_LLAMA_WOW_CONFIG_H
