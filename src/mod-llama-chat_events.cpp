...
namespace
{
    LlamaBotEventChatter eventChatter;
...
void LlamaBotEventChatter::DispatchGameEvent(Player* source, std::string type, std::string detail)
{
    ...
    const bool sourceIsBot = LlamaIsBotPlayer(source);
    ...
    for (Player* bot : candidateBots)
    {
        ...
        const bool partyAudience =
            bot->GetGroup() && !g_DisableForParty && LlamaGroupHasRealPlayer(bot);
...
void ChatOnQuest::OnPlayerCompleteQuest(Player* player, Quest const* quest)
{
        ...
        player->GetMap() && player->GetMap()->IsDungeon())
    {
        eventChatter.DispatchGameEvent(
            ...
            SafeFormat("{} in {}", quest->GetTitle(), LlamaContinentName(player)));
    }
}
