#ifndef MOD_LLAMA_HTTPCLIENT_H
#define MOD_LLAMA_HTTPCLIENT_H
...
// which is why an unsupported think mode looked identical to a dead Llama.
struct LlamaHttpResult
{
...
public:
    ...
    LlamaHttpResult PostEx(const std::string& url, const std::string& jsonData,
    ...
    LlamaHttpResult PostExWithToken(const std::string& url, const std::string& jsonData,
                                     const std::string& bearerToken, int timeoutOverride = 0);
    LlamaHttpResult GetEx(const std::string& url, int timeoutOverride = 0);
