...
namespace
{
    ...
    bool ChooseDestination(Player* bot, const LlamaWorldSnapshot& world, bool guildTopic,
                           ...
                           uint32_t& outChannelId)
    {
        ...
        if (bot->GetGroup() && !g_DisableForParty && LlamaGroupHasRealPlayer(bot))
        {
        ...
        if (!g_DisableForCustomChannels && world.RealPlayerInZoneAndFaction(bot))
        {
            ...
            auto addChannel = [&](uint32_t chanId, bool enabled)
            {
                ...
                Channel* ch = LlamaResolveZoneChannel(bot, chanId);
                if (!ch)
                    return;
    ...
    std::string BuildRandomChatterPrompt(Player* bot, const std::string& environmentInfo)
    {
        ...
        std::string prompt = SafeFormat(
            ...
            fmt::arg("bot_map", LlamaContinentName(bot)),
...
void LlamaRandomChatter_ForgetBot(ObjectGuid botGuid)
{
    std::lock_guard<std::mutex> lock(g_scheduleMutex);
    g_nextRandomChatTime.erase(botGuid.GetRawValue());
}

LlamaBotRandomChatter::LlamaBotRandomChatter() : WorldScript("LlamaBotRandomChatter") { }

void LlamaBotRandomChatter::OnUpdate(uint32 diff)
{
    if (!g_Enable)
        return;
...
void LlamaBotRandomChatter::HandleRandomChatter()
{
    ...
    for (auto const& itr : allPlayers)
    {
        ...
        request.kind        = LlamaRequestKind::RandomChatter;
        request.triggerBotReplies = true;

        if (LlamaDispatch_Submit(std::move(request)))
