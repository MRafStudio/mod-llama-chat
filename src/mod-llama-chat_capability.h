#ifndef MOD_LLAMA_CAPABILITY_H
#define MOD_LLAMA_CAPABILITY_H
...
//
//   CAN this model think?   -- answered by probing Llama, cached per
...
enum class LlamaThinkPolicy : uint8_t
{
...
enum class LlamaRequestKind : uint8_t
{
    ChatReply = 0,
    RandomChatter,
    EventChatter,
...
enum class LlamaThinkSupport : uint8_t
{
    Unknown = 0,      // probe has not completed yet
    Supported,
    Unsupported,
    ProbeFailed,      // could not reach Llama; treated as unsupported
};

// Kick off (or re-run) the capability probe. Runs on a background thread so a
// missing or slow Llama cannot stall worldserver startup. Safe to call again
// after a config reload; re-probes when url or model changed, or when forced.
void LlamaCapability_Init(bool force = false);
...
//
// Llama takes either a bool or, on models with reasoning-effort levels
...
// Self-heal: Llama refused a string reasoning level, so this model takes the
...
// True when an HTTP failure is Llama telling us the model cannot think.
...
// harmony-format builds reason unconditionally, and Llama counts those tokens
...
// Human-readable summary for the .llama status command.
