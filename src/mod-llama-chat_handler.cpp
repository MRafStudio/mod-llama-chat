...
static size_t LlamaFindBotNameMention(const std::string& msg, const std::string& botName)
{
...
static bool LlamaIsDirectAddress(Player* bot, Player* speaker, ChatChannelSourceLocal source,
                                  ...
                                  const std::string& scopeKey)
{
    ...
    if (LlamaFindBotNameMention(msg, bot->GetName()) != std::string::npos)
        return true;
...
namespace
{
    ...
    struct LlamaNearbyCreatureCheck
    {
        LlamaNearbyCreatureCheck(WorldObject const* obj, float range)
...
bool PlayerBotChatHandler::OnPlayerCanUseChat(Player* player, uint32_t type, uint32_t lang, std::string& msg, Player* receiver)
{
    ...
    if (type == CHAT_MSG_WHISPER)
    {
        ...
        // Check if sender is a bot - if so, don't trigger Llama responses for bot-to-bot whispers
...
std::vector<std::string> ChatHandler_GetVisibleLocations(Player* bot, float radius = 40.0f)
{
    ...
    LlamaNearbyCreatureCheck creatureCheck(bot, radius);
    Acore::CreatureListSearcher<LlamaNearbyCreatureCheck> creatureSearcher(bot, creatures, creatureCheck);
...
void PlayerBotChatHandler::ProcessChat(Player* player, uint32_t /*type*/, uint32_t lang, std::string& msg, ChatChannelSourceLocal sourceLocal, Channel* channel, Player* receiver, uint8_t chainDepth)
{
    ...
    if(g_DebugEnabled)
    {
        LOG_INFO("server.loading",
                "[Llama Chat] Player {} sent msg: '{}' | Source: {} | Channel Name: {} | Channel ID: {} | Receiver: {}",
    ...
    for (const std::string& blacklist : g_BlacklistCommands)
    {
        if (startsWithWord(trimmedMsg, blacklist))
        {
            if (g_DebugEnabled)
                LOG_INFO("server.loading",
                         "[Llama Chat] Message starts with '{}' (blacklisted). Skipping bot responses.",
                         blacklist);
            return;
        }
    }
    ...
    if (sourceLocal == SRC_WHISPER_LOCAL && receiver != nullptr)
    {
        ...
        // Skip bot-to-bot whispers to prevent Llama responses
        if (senderIsBot)
        {
            return;
        }
    ...
    else if (channel != nullptr)
    {
        ...
        for (auto const& playerItr : allPlayers)
        {
            ...
            if (LlamaIsRealPlayer(candidateReal))
            {
    ...
    else
    {
        ...
        // Whole-word, case-insensitive name match; see LlamaFindBotNameMention.
        auto isBotNameMentioned = [&trimmedMsg](const std::string& botName) -> size_t {
            return LlamaFindBotNameMention(trimmedMsg, botName);
        };
        ...
        else
        {
            for (Player* bot : candidateBots)
            {
                ...
                const bool botIsAddressed =
                    LlamaIsDirectAddress(bot, player, sourceLocal, trimmedMsg, senderIsBot, scopeKey);
    ...
    for (Player* bot : finalCandidates)
    {
        ...
        request.kind = (g_RoleplayEnable && g_RoleplayStrictness >= 1)
                           ? LlamaRequestKind::RoleplayReply
                           : LlamaRequestKind::ChatReply;
...
{  
    ...
    std::string botMapName          = LlamaContinentName(bot);
    ...
    std::string relationAxes     = GetRelationAxesPromptAddition(bot, player);
...
void LlamaChatMaintenance::OnPlayerLogout(Player* player)
{
    if (!player)
        return;
    ...
    LlamaRandomChatter_ForgetBot(guid);
