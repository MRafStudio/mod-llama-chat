#ifndef MOD_LLAMA_WOW_UTILS_H
#define MOD_LLAMA_WOW_UTILS_H
...
// Safe formatting utility for the LlamaWow Chat module.
...
inline std::string SafeFormat(const std::string& templ, Args&&... args) {
    try {
        ...
        LOG_ERROR("server.loading", "[LlamaWow Chat] Format error: {} | Template: {}", e.what(), templ);
        return "[Format Error]";
    }
}
