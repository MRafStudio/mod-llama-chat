...
#include "mod-llama-wow_governor.h"
#include "mod-llama-wow_response.h"
...
static size_t LlamaWowFindBotNameMention(const std::string& msg, const std::string& botName)
{
...
static bool LlamaWowIsDirectAddress(Player* bot, Player* speaker, ChatChannelSourceLocal source,
                                  ...
                                  const std::string& scopeKey)
{
    ...
    if (LlamaWowFindBotNameMention(msg, bot->GetName()) != std::string::npos)
        return true;
...
namespace
{
    ...
    struct LlamaWowNearbyCreatureCheck
    {
        LlamaWowNearbyCreatureCheck(WorldObject const* obj, float range)
...
Channel* GetValidChannel(uint32_t teamId, const std::string& channelName, Player* player)
{
    ...
    if (!channel)
    {
        if(g_DebugEnabled)
        {
            LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] Channel '{}' not found for team {}", channelName, teamId);
        }
    }
    return channel;
}
...
bool PlayerBotChatHandler::OnPlayerCanUseChat(Player* player, uint32_t type, uint32_t lang, std::string& msg, Player* receiver)
{
    ...
    if (type == CHAT_MSG_WHISPER)
    {
        ...
        // Check if sender is a bot - if so, don't trigger LlamaWow responses for bot-to-bot whispers
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] OnPlayerCanUseChat called: player={}, type={}, receiver={}",
...
void SaveBotConversationHistoryToDB()
{
    ...
    for (const PendingPair& entry : pending)
    {
        ...
        auto flushValues = [&]()
        {
            ...
            trans->Append("INSERT IGNORE INTO mod_llama_wow_history "
        ...
        trans->Append(SafeFormat(
            "DELETE FROM mod_llama_wow_history "
            "WHERE bot_guid = {0} AND player_guid = {1} AND id < ("
                "SELECT keep_id FROM ("
                    "SELECT id AS keep_id FROM mod_llama_wow_history "
    ...
    if (g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow",
                 "[LlamaWow Chat] Saved {} new conversation turn(s) across {} bot/player pair(s).",
...
void DeleteBotConversationHistoryFromDB(uint64_t botGuid)
{
    CharacterDatabase.Execute(SafeFormat(
        "DELETE FROM mod_llama_wow_history WHERE bot_guid = {}", botGuid));
}
...
void ProcessBotChatMessage(Player* bot, const std::string& msg, ChatChannelSourceLocal sourceLocal, Channel* channel, uint8_t chainDepth)
{
    ...
    if (!channel && sourceLocal == SRC_GENERAL_LOCAL)
    {
        ...
        if (cMgr)
        {
            ...
            {
                if (channel)
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] ProcessBotChatMessage: Found General channel for bot {}", bot->GetName());
                else
                    LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] ProcessBotChatMessage: Could not find General channel for bot {}", bot->GetName());
            }
        }
    }
    ...
    switch (sourceLocal)
    {
        ...
        case SRC_GENERAL_LOCAL:
            ...
            if (!canSendMessage && g_DebugEnabled)
                LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] No bot replies to {} in General - no channel found", bot->GetName());
            break;
        ...
        case SRC_OFFICER_LOCAL:
            ...
            if (bot->GetGuildId() != 0)
            {
                ...
                if (guild)
                {
                    LlamaWowWorldSnapshot world;
                    world.Build();
                    const bool hasRealPlayer = world.GuildHasRealPlayer(bot->GetGuildId());
                    canSendMessage = hasRealPlayer;
                    if (!canSendMessage && g_DebugEnabled)
                        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] No bot replies to {} in Guild - no real players online in guild", bot->GetName());
                }
                else
                {
                    canSendMessage = false;
                    if (g_DebugEnabled)
                        LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] No bot replies to {} in Guild - guild not found", bot->GetName());
                }
            }
            else
            {
                canSendMessage = false;
                if (g_DebugEnabled)
                    LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] No bot replies to {} in Guild - not in a guild", bot->GetName());
            }
            break;
        ...
        case SRC_RAID_LOCAL:
            ...
            if (bot->GetGroup())
            {
                ...
                for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
                {
                    ...
                    if (LlamaWowIsRealPlayer(member))
                    {
                        hasRealPlayer = true;
                        break;
                    }
                ...
                canSendMessage = hasRealPlayer;
                if (!canSendMessage && g_DebugEnabled)
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] No bot replies to {} in Party - no real players in group", bot->GetName());
            }
            else
            {
                canSendMessage = false;
                if (g_DebugEnabled)
                    LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] No bot replies to {} in Party - not in a group", bot->GetName());
            }
            break;
    ...
    if (!canSendMessage)
    {
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow",
                     "[LlamaWow Chat] Not propagating {}'s {} line to other bots - no audience.",
...
std::vector<std::string> ChatHandler_GetVisibleLocations(Player* bot, float radius = 40.0f)
{
    ...
    LlamaWowNearbyCreatureCheck creatureCheck(bot, radius);
    Acore::CreatureListSearcher<LlamaWowNearbyCreatureCheck> creatureSearcher(bot, creatures, creatureCheck);
...
{
    if (player == nullptr) {
        LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] ProcessChat: player is null");
        return;
    }
    if (msg.empty()) {
        return;
    ...
    if(g_DebugEnabled)
    {
        LOG_INFO("server.loading",
                "[LlamaWow Chat] Player {} sent msg: '{}' | Source: {} | Channel Name: {} | Channel ID: {} | Receiver: {}",
    ...
    for (const std::string& blacklist : g_BlacklistCommands)
    {
        if (startsWithWord(trimmedMsg, blacklist))
        {
            if (g_DebugEnabled)
                LOG_INFO("server.loading",
                         "[LlamaWow Chat] Message starts with '{}' (blacklisted). Skipping bot responses.",
                         blacklist);
            return;
        }
    }
    ...
    if (sourceLocal == SRC_GENERAL_LOCAL && g_DisableForCustomChannels)
    {
        if (g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Custom channels are disabled, skipping");
        }
        return;
    }
    ...
    if ((sourceLocal == SRC_SAY_LOCAL || sourceLocal == SRC_YELL_LOCAL) && g_DisableForSayYell)
    {
        if (g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Say/Yell channels are disabled, skipping");
        }
        return;
    }
    ...
    if ((sourceLocal == SRC_GUILD_LOCAL || sourceLocal == SRC_OFFICER_LOCAL) && g_DisableForGuild)
    {
        if (g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Guild channels are disabled, skipping");
        }
        return;
    }
    ...
    if ((sourceLocal == SRC_PARTY_LOCAL || sourceLocal == SRC_RAID_LOCAL) && g_DisableForParty)
    {
        if (g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Party/Raid channels are disabled, skipping");
        }
        return;
    }
    ...
    else
    {
        ...
        if (!Governor_ChainDepthAllowed(chainDepth))
        {
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Chain depth {} reached the limit; not continuing.",
                         chainDepth);
            return;
        }
        ...
        if (!Governor_HasRecentHuman(scopeKey))
        {
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] No real player has spoken in {} recently; "
    ...
    if (sourceLocal == SRC_WHISPER_LOCAL && receiver != nullptr)
    {
        ...
        if (!g_EnableWhisperReplies)
        {
            if(g_DebugEnabled)
            {
                LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Whisper replies are disabled, skipping");
            }
            return;
        }
        ...
        if(g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Processing whisper from {} to {}", 
                    player->GetName(), receiver->GetName());
        }
        
        // Skip bot-to-bot whispers to prevent LlamaWow responses
        if (senderIsBot)
        {
            return;
        }
        ...
        if (receiverAI && receiverAI->IsBotAI())
        {
            ...
            if(g_DebugEnabled)
            {
                LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Found eligible bot {} for whisper", receiver->GetName());
            }
        }
        else if(g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Whisper target {} is not a bot or has no AI", receiver->GetName());
        }
    }
    else if (channel != nullptr)
    {
        ...
        if(g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Processing channel message in '{}' (ID: {})", 
        ...
        if (!channel)
        {
            if(g_DebugEnabled)
            {
                LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] Channel is null, cannot process channel message");
            }
            return;
        }
        ...
        for (auto const& playerItr : allPlayers)
        {
            ...
            if (LlamaWowIsRealPlayer(candidateReal))
            {
        ...
        if (!hasRealPlayerInChannel)
        {
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] No real players in channel '{}'; skipping.",
                         channel->GetName());
            return;
        }
        ...
        for (auto const& itr : allPlayers)
        {
            ...
            if (!candidate->IsInChannel(channel))
            {
                if(g_DebugEnabled)
                {
                    //LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} not in channel '{}', skipping", candidate->GetName(), channel->GetName());
                }
                continue;
            }
            ...
            if (candidate->GetTeamId() != player->GetTeamId())
            {
                if (!isGlobalChannel)
                {
                    if(g_DebugEnabled)
                    {
                        //LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] Bot {} FAILED faction check - Bot: {}, Player: {}, Channel: '{}'", candidate->GetName(), (int)candidate->GetTeamId(), (int)player->GetTeamId(), channel->GetName());
                    }
            ...
            if (!hasRealPlayerInChannel)
            {
                if(g_DebugEnabled)
                {
                    //LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} skipped - no real players in channel '{}'", candidate->GetName(), channel->GetName());
                }
                continue;
            }
            ...
            if(g_DebugEnabled)
            {
                // LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] VERIFIED eligible bot {} in channel '{}' - Distance: {:.2f}, Zone match: {}", candidate->GetName(), channel->GetName(), candidate->GetDistance(player), (candidate->GetZoneId() == player->GetZoneId()));
            }
        }
        
        if(g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Found {} bots in channel instance '{}'", 
    ...
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] {} bots not eligible for {} (distance/guild/party checks failed)", 
    ...
    if(g_DebugEnabled)
    {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Sender: {} ({}), Channel: {}, Depth: {}, Reply Chance: {}%, Candidate Bots: {}",
    ...
    if (sourceLocal == SRC_WHISPER_LOCAL)
    {
        if (!candidateBots.empty())
        {
            ...
            if (!(g_DisableRepliesInCombat && whisperBot->IsInCombat()))
            {
                ...
                if(g_DebugEnabled)
                {
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Whisper: Bot {} selected to respond", whisperBot->GetName());
                }
            }
        }
    }
    else
    {
        ...
        // Whole-word, case-insensitive name match; see LlamaWowFindBotNameMention.
        auto isBotNameMentioned = [&trimmedMsg](const std::string& botName) -> size_t {
            return LlamaWowFindBotNameMention(trimmedMsg, botName);
        };
        ...
        for (Player* bot : candidateBots)
        {
            ...
            if (pos != std::string::npos)
            {
                ...
                if(g_DebugEnabled)
                {
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} mentioned at position {} in message", bot->GetName(), pos);
                }
            }
        }
        ...
        if (!mentionedBots.empty())
        {
            ...
            if (!(g_DisableRepliesInCombat && chosen->IsInCombat()))
            {
                ...
                if(g_DebugEnabled)
                {
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} selected (mentioned first at position {})", 
        ...
        else
        {
            for (Player* bot : candidateBots)
            {
                if (g_DisableRepliesInCombat && bot->IsInCombat())
                {
                    if(g_DebugEnabled)
                    {
                        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} skipped - in combat", bot->GetName());
                    }
                    continue;
                }
                ...
                const bool botIsAddressed =
                    LlamaWowIsDirectAddress(bot, player, sourceLocal, trimmedMsg, senderIsBot, scopeKey);
                ...
                if (roll < botChance)
                {
                    ...
                    if(g_DebugEnabled)
                    {
                        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} PASSED chance roll ({} < {}%{})", bot->GetName(), roll, botChance, botIsAddressed ? ", addressed" : "");
                    }
                }
                else if(g_DebugEnabled)
                {
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Bot {} FAILED chance roll ({} >= {}%{})", bot->GetName(), roll, botChance, botIsAddressed ? ", addressed" : "");
                }
            }
        }
    }
    ...
    if (finalCandidates.empty())
    {
        if(g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] *** NO BOTS RESPONDING *** to {} from {} in {} channel. "
            ...
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] No eligible bots found to respond to message '{}'. "
    ...
    if (finalCandidates.size() > g_MaxBotsToPick)
    {
        ...
        if(g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Limiting {} bots to {} (MaxBotsToPick)", finalCandidates.size(), countToPick);
        }
    ...
    if(g_DebugEnabled && !finalCandidates.empty())
    {
        ...
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] *** {} BOTS RESPONDING *** to {} from {} in {}: [{}]",
    ...
    for (Player* bot : finalCandidates)
    {
        ...
        if (!Governor_CanSend(bot->GetGUID(), scopeKey, directAddress))
        {
            if (g_DebugEnabled)
                LOG_INFO("module.mod_llama_wow",
                         "[LlamaWow Chat] Bot {} skipped: cooldown or rate limit.", bot->GetName());
            continue;
        }
        ...
        request.kind = (g_RoleplayEnable && g_RoleplayStrictness >= 1)
                           ? LlamaWowRequestKind::RoleplayReply
                           : LlamaWowRequestKind::ChatReply;
        ...
        if (!LlamaWowDispatch_Submit(std::move(request)) && g_DebugEnabled)
        {
            LOG_INFO("module.mod_llama_wow",
                     "[LlamaWow Chat] Bot {} reply dropped: dispatcher queue full.",
                     bot->GetName());
        }
    }
}
...
static bool IsBotEligibleForChatChannelLocal(Player* bot, Player* player, ChatChannelSourceLocal source, Channel* channel, Player* receiver)
{
    ...
    {
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] IsBotEligible: FAILED basic check - bot={}, player={}, same={}", 
    ...
    {
        if (g_DebugEnabled)
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] IsBotEligible: Bot {} FAILED - no PlayerbotAI", bot->GetName());
        return false;
    }
    ...
    if (channel)
    {
        ...
        if (!channel)
        {
            if(g_DebugEnabled)
            {
                LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] IsBotEligibleForChatChannelLocal: Channel is null");
            }
            return false;
        }
        ...
        if (!candidateChannel || candidateChannel != channel)
        {
            if(g_DebugEnabled)
            {
                LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] IsBotEligibleForChatChannelLocal: Bot {} not in same channel instance '{}' - Bot team: {}, Channel ptr: {} vs {}", 
        ...
        if (bot->GetTeamId() != player->GetTeamId())
        {
            ...
            if (!isGlobalChannel)
            {
                if(g_DebugEnabled)
                {
                    LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] IsBotEligibleForChatChannelLocal: Bot {} different faction from player - Bot: {}, Player: {}, Channel: '{}'", bot->GetName(), (int)bot->GetTeamId(), (int)player->GetTeamId(), channel->GetName());
                }
                return false;
            }
        }
...
{  
    ...
    if (g_ChatPromptTemplate.empty()) {
        LOG_ERROR("module.mod_llama_wow", "[LlamaWow Chat] GenerateBotPrompt: template is empty");
        return "";
    }
    ...
    uint8 personalityLocale         = player->GetSession() ? player->GetSession()->GetSessionDbcLocale() : uint8(LOCALE_enUS);
    std::string personalityPrompt   = GetPersonalityPromptAddition(personality, personalityLocale);
    ...
    std::string botMapName          = LlamaWowContinentName(bot);
    ...
    if (g_EnableRAG && g_RAGSystem) {
        ...
        if (g_DebugEnabled) {
            LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] RAG Debug - Enabled: {}, System: {}, Message: '{}', Results: {}, Content length: {}",
                g_EnableRAG, (void*)g_RAGSystem, playerMessage, ragResults.size(), ragContent.length());
        }
    } else if (g_DebugEnabled) {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] RAG Debug - Not enabled or no system - Enabled: {}, System: {}",
    ...
    if (g_DebugEnabled && g_DebugShowFullPrompt) {
        LOG_INFO("module.mod_llama_wow", "[LlamaWow Chat] Full prompt sent to bot {} for player {}: {}", botName, playerName, prompt);
    }
...
std::string BuildEmoteReactionPrompt(Player* bot, Player* player, uint32_t textEmote)
{
    ...
    uint8 personalityLocale             = bot->GetSession() ? bot->GetSession()->GetSessionDbcLocale() : uint8(LOCALE_enUS);
    const std::string personalityPrompt = GetPersonalityPromptAddition(personality, personalityLocale);
...
void LlamaWowChatMaintenance::OnPlayerLogout(Player* player)
{
    if (!player)
        return;
    ...
    LlamaWowRandomChatter_ForgetBot(guid);
