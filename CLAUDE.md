...
2. Fill an `LlamaChatRequest` — resolve every value you need (names, guids,
   scope key) into it now, so the worker never has to touch a `Player`.
3. `LlamaDispatch_Submit(std::move(request))`.
4. Delivery happens for you in `LlamaDispatch_Update()`, on the world thread.

Do **not** call `QueryLlama()` directly from anywhere that could be the world
...
A worker thread must not touch `g_LlamaUrl`, `g_LlamaModel`,
`g_LlamaSystemPrompt`, `g_LlamaStop`, `g_LlamaSeed`,
`g_SentimentAnalysisPrompt` or any other `std::string` global.

`.llama reload` reassigns those on the world thread. Reassigning a
...
- endpoint settings go through `LlamaConfig_Snapshot()`, published under a
  mutex by `LlamaConfig_Publish()` at the end of `LoadLlamaChatConfig()`
...
Use `LlamaResolveZoneChannel(bot, ChatChannelId::X)`, which matches on channel
